...
function generatePromptText(branchId, snap = null, sc = null) {
    ...
    const ctx = scores.total >= 6.5
        ? "الأداء متوسط ومقبول ولكن يحتاج تحسين ملحوظ"
        : scores.ptsSafety < 3
            ? "هناك مشكلة جدية في السلامة تستوجب التنبيه العاجل"
            : scores.ptsComplaints < 1
                ? "ارتفاع مقلق في الشكاوى يستدعي تدخلاً فورياً"
    ...
    const mgtParts = [];
    if (data.mName) mgtParts.push(`المديرة ${data.mName}`);
    if (data.dName) mgtParts.push(`نائبتها ${data.dName}`);
...
function openArticleModal(branchId, ts = null) {
    ...
    if (data.mName) sp.push(`المديرة: ${data.mName}`);
    else if (data.dName) sp.push(`النائبة: ${data.dName}`);
...
function openAnnouncementModal() {
    ...
    document.getElementById('articleModalBody').innerHTML = `
        <div>
            ...
            <input type="text" id="announcementTitle" class="w-full p-3 rounded-lg glass-input text-sm mb-4" placeholder="أدخل عنوان الإعلان">
        </div>
        <div>
            <label class="block text-sm font-bold text-slate-700 mb-2">نص الإعلان</label>
            <textarea id="announcementBody" rows="8" class="w-full p-4 rounded-xl glass-input text-sm leading-relaxed resize-y" placeholder="اكتب نص الإعلان هنا..."></textarea>
        </div>
        <div class="flex gap-3 pt-2">
...
function openWeeklyModal() {
    ...
    document.getElementById('weeklyModalBody').innerHTML = `
        ...
        <div>
            <label class="block text-sm font-bold text-slate-700 mb-2">المقال الاسبوعي (الصقه هنا)</label>
            <textarea id="weeklyArticleText" rows="6" class="w-full p-4 rounded-xl glass-input text-sm leading-relaxed resize-y" placeholder="الصق المقال الاسبوعي هنا..."></textarea>
        </div>
...
function generateWeeklyPromptUI() {
    ...
    box.textContent = generateWeeklyPrompt(rm);
...
async function saveWeeklyArticle() {
    ...
    closeWeeklyModal();
    generateNewspaper();
    showCopyToast('تم حفظ التحديث الاسبوعي');
}
...
async function generateOpinionLink() {
...
function copyOpinionLink() {
    _fallbackCopy(document.getElementById('opinionLinkUrl').value);
...
async function submitOpinionArticle() {
    ...
    const ts = Date.now(), dateStr = new Date(ts).toISOString().split('T')[0];
    globalArticles.push({
        type: 'opinion', title, body, text: `${title}\n${body}`, timestamp: ts, dateStr,
...
function initCarousel() {
    ...
    sorted.forEach((i, idx) => {
        ...
        const mgt = [];
        if (data.mName) mgt.push(data.mName);
        if (data.dName) mgt.push(data.dName);
        ...
        card.onclick = (e) => {
            ...
            isAdminLoggedIn
                ? (document.getElementById('branchSelector').value = i, loadAdminData(), document.getElementById('adminModal').style.display = 'flex')
...
function setupCarouselDots() {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        b.className = 'carousel-dot w-2.5 h-2.5 rounded-full bg-slate-300/80 transition-all duration-300 border border-white/50';
        dc.appendChild(b);
    }
    updateActiveDot();
}
...
function scrollToCard(index) {
    const c = document.getElementById('branchesCarousel');
    if (c.children[index]) c.children[index].scrollIntoView({ behavior:'smooth', block:'nearest', inline:'center' });
}
...
function updateActiveDot() {
    ...
    for (let i = 0; i < c.children.length; i++) {
        const cc = c.children[i].getBoundingClientRect();
        const d = Math.abs(cCx - (cc.left + cc.width / 2));
        if (d < minD) { minD = d; ci = i; }
    }
    dots.forEach((d, i) => {
        d.className = i === ci
            ? 'carousel-dot w-6 h-2.5 rounded-full bg-slate-800 transition-all duration-300 border border-white/50'
            : 'carousel-dot w-2.5 h-2.5 rounded-full bg-slate-300/80 transition-all duration-300 border border-white/50';
    });
}
...
function openBulletinPage(branchId, timestamp = null) {
    ...
    if (timestamp) {
        ...
        if (articleData && articleData.snapshot) {
            data = articleData.snapshot; scores = articleData.scores || calcScores(data);
            dateStr = formatFullDateArabic(new Date(articleData.timestamp));
        } else {
            data = branchesData[branchId]; scores = calcScores(data); dateStr = formatFullDateArabic(new Date());
        }
    } else {
        data = branchesData[branchId]; scores = calcScores(data);
        articleData = getArticleData(branchId); dateStr = formatFullDateArabic(new Date());
    }
    ...
    currentBulletinData = {
        ...
        head: aHead || `تقرير أداء فرع ${data.bName}`,
        lead: aLead || `مؤشر الأداء العام: ${scores.total}/11 — تصنيف: ${tier.label}`,
        performanceLabel: tier.label, dateStr, isGlobal: false
    };
    document.getElementById('bulletinBreadcrumb').textContent      = 'التقرير الدوري';
    ...
    showPage('bulletin');
    const cel = document.getElementById('bulletinContent');
...
function buildBulletinHTML(data, scores, tier, text, rv, rc, dateStr, pct, branchId, hasArticle, tsToPass) {
    ...
    const adminBtns = isAdminLoggedIn ? `<div class="flex gap-2 flex-wrap">
        ...
        <button onclick="openArticleModal(${branchId},${tsP})" class="btn-solid-dark text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5">
            ...
            ${hasArticle ? 'تعديل المقال' : 'كتابة مقال'}</button>
    ...
    const waiting = [];
    if (data.mName) waiting.push(`المديرة ${data.mName}`);
    if (data.dName) waiting.push(`نائبتها ${data.dName}`);
        ...
        : `<div class="mb-8 ${isAdminLoggedIn ? 'bg-amber-50/50 border-amber-200' : 'bg-slate-100/40 border-slate-200'} border rounded-2xl p-6">
            <div class="flex items-start gap-4">
                ...
                <div>${isAdminLoggedIn
                    ...
                    : `<p class="text-slate-800 text-lg font-bold mt-1">${waitText}</p>`}
                </div>
            </div>
        </div>`;
    let teamHTML = '';
...
function openGlobalBulletinPage(ts) {
    ...
    currentBulletinData = {
        ...
        head, lead: bodyTxt.substring(0, 100),
        performanceLabel: '', dateStr, isGlobal: true, globalTs: ts
    };

    document.getElementById('bulletinBreadcrumb').textContent      = typeLabels[art.type] || 'مقال';
    document.getElementById('bulletinBreadcrumbBranch').textContent = head.length > 35 ? head.substring(0, 35) + '…' : head;

    history.pushState('', '', `#global-${ts}`);
    updateOGTags(`${head} - أي آم سبيشل`, bodyTxt.substring(0, 120), location.href);

    const delBtn = isAdminLoggedIn
    ...
    if (art.type === 'weekly' && art.branchSnapshots) {
        const rows = Object.entries(art.branchSnapshots).map(([id, d]) => {
            ...
            return `<div class="flex items-center justify-between py-2.5 border-b border-white/30 last:border-0">
                <span class="font-bold text-slate-800 text-sm">فرع ${d.bName}</span>
                <div class="flex items-center gap-2">
                    <span class="font-black text-slate-900 text-sm">${s.total}<span class="text-xs font-medium text-slate-400">/11</span></span>
                    <span class="badge ${t.badgeCls} text-[10px]">${t.label}</span>
                </div>
            </div>`;
        }).join('');
        extraContent = `<div class="mt-8 glass-panel rounded-2xl p-5">
            <h2 class="text-xs font-black text-slate-600 mb-4 uppercase tracking-wider">ملخص الفروع</h2>
            ${rows}
        </div>`;
    } else if (art.type === 'opinion' && art.authorName) {
        extraContent = `<div class="mt-6 glass-panel rounded-xl p-4 flex items-center gap-3">
            <div class="w-10 h-10 bg-white/70 rounded-full flex items-center justify-center font-black text-slate-800 text-base border border-white">${art.authorName.charAt(0)}</div>
            <div>
                <p class="font-black text-slate-900 text-sm">${art.authorName}</p>
    ...
    document.getElementById('bulletinContent').innerHTML = `
        <div class="mb-8">
            <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
                <div class="flex items-center gap-3">
                    <span class="badge ${typeBadges[art.type] || 'badge-slate'}">${typeLabels[art.type] || ''}</span>
                    <span class="text-slate-500 bg-white/40 px-3 py-1 rounded-full text-sm font-bold border border-white/50">${dateStr}</span>
                </div>
                ${delBtn}
            </div>
...
function generateNewspaper() {
    ...
    for (let i = 1; i <= 6; i++) {
        ...
        if (arts && Array.isArray(arts)) {
            arts.forEach(a => all.push({
                branchId: i, type: a.type || 'performance', article: a.text,
                timestamp: a.timestamp, dateStr: a.dateStr,
                snapshot: a.snapshot || branchesData[i],
                scores:   a.scores   || calcScores(branchesData[i])
            }));
        }
    }
    globalArticles.forEach(a => all.push({
        branchId: null, type: a.type, article: a.text, title: a.title || null,
        timestamp: a.timestamp, dateStr: a.dateStr,
        authorName: a.authorName || null, authorBio: a.authorBio || null,
        snapshot: null, scores: null
    }));
    all.sort((a, b) => b.timestamp - a.timestamp);

    if (!all.length) {
        tl.innerHTML = `<div class="relative">
            <div class="absolute -right-6 top-0 w-4 h-4 rounded-full bg-slate-800 border-4 border-white/60"></div>
            ...
            <p class="text-center text-slate-600 bg-white/40 rounded-xl py-8 font-bold border border-white/60">لا توجد تقارير</p>
        </div>`;
        return;
    }
    ...
    Object.keys(grouped).sort((a, b) => new Date(b) - new Date(a)).forEach(dk => {
        ...
        grouped[dk].forEach(item => {
            const card  = document.createElement('div');
            card.className = 'glass-panel p-6 rounded-2xl flex flex-col transition-transform hover:-translate-y-1 border border-white/60 newspaper-card';
            const delBtn = isAdminLoggedIn
                ? `<button onclick="event.stopPropagation();${item.branchId ? `deleteArticle(${item.branchId},${item.timestamp})` : `deleteGlobalArticle(${item.timestamp})`}" class="w-7 h-7 flex items-center justify-center btn-outline btn-outline-rose rounded-full" title="حذف">${DEL_SVG}</button>`
                : '';
            ...
            if (item.type === 'performance' && item.branchId) {
                ...
                card.innerHTML = `
                    <div class="flex-1">
                        <div class="flex justify-between items-start mb-3">
                            <span class="badge ${tier.badgeCls}">الأداء ◂ ${item.snapshot.bName}</span>${delBtn}
                        </div>
                        <h3 onclick="openBulletinPage(${item.branchId},${item.timestamp})"
                            class="font-black text-xl mt-5 mb-3 leading-snug text-slate-900 cursor-pointer hover:text-blue-700 transition">${lines[0] || ''}</h3>
                        <p class="text-slate-700 text-sm font-bold leading-relaxed bg-white/30 p-2 rounded">${lines[1] || ''}</p>
                    </div>
                    <div class="mt-5 pt-5 border-t border-white/40">
                        <div class="flex justify-between items-center bg-white/40 px-3 py-2 rounded-lg text-slate-800 font-black">
                            <span>النقاط</span>
                            <span class="btn-solid-dark px-3 py-1 rounded text-sm">${item.scores.total} ◠</span>
                        </div>
                    </div>`;
                ...
                card.innerHTML = `
                    <div class="flex-1">
                        <div class="flex justify-between items-start mb-3">
                            <span class="badge badge-emerald">نظرة عن كثب</span>${delBtn}
                        </div>
                        <h3 onclick="openGlobalBulletinPage(${item.timestamp})"
                            class="font-black text-xl mt-5 mb-3 leading-snug text-slate-900 cursor-pointer hover:text-blue-700 transition">${lines[0] || 'التحديث الاسبوعي'}</h3>
                ...
                card.innerHTML = `
                    <div class="flex-1">
                        <div class="flex justify-between items-start mb-3">
                            <span class="badge badge-amber">إعلان</span>${delBtn}
                        </div>
                        <h3 onclick="openGlobalBulletinPage(${item.timestamp})"
                            class="font-black text-xl mt-5 mb-3 leading-snug text-slate-900 cursor-pointer hover:text-blue-700 transition">${lines[0] || 'إعلان'}</h3>
                        <p class="text-slate-700 text-sm font-bold leading-relaxed bg-white/30 p-2 rounded">${excerpt}${excerpt.length >= 120 ? '...' : ''}</p>
                    </div>`;
                ...
                card.innerHTML = `
                    <div class="flex-1">
                        <div class="flex justify-between items-start mb-3">
                            <span class="badge badge-rose">رأي</span>${delBtn}
                        </div>
                        <h3 onclick="openGlobalBulletinPage(${item.timestamp})"
                            class="font-black text-xl mt-5 mb-3 leading-snug text-slate-900 cursor-pointer hover:text-blue-700 transition">${lines[0] || 'مقال رأي'}</h3>
                        <p class="text-slate-700 text-sm font-medium leading-relaxed bg-white/30 p-2 rounded">${excerpt}${bodyRaw.length > 120 ? '...' : ''}</p>
                        ${byline}
                    </div>`;
            }
...
function showPage(id) {
    ...
    const mw = document.getElementById('mainPageWrapper');
    const bp = document.getElementById('bulletinPage');
    mw.style.display = isMain ? 'flex' : 'none';
    mw.style.flexDirection = 'column';
    bp.style.display = isMain ? 'none' : 'flex';
...
function goToMainPage() {
    ...
    updateOGTags('التقارير - آي أم سبيشل', 'تقرير الأداء', location.href);
}
...
function shareBulletinPage() {
    ...
    const head = currentBulletinData?.head || 'التقارير';
    const lead = currentBulletinData?.lead || '';
    const isGlobal = currentBulletinData?.isGlobal;
    const typeLabel = currentBulletinData?.bName || '';
    const perfLabel = currentBulletinData?.performanceLabel || '';

    const msg = isGlobal
        ? `*${typeLabel}*\nأي آم سبيشل\n\n${head}\n\n${url}`
        : `*تقرير أداء فرع ${typeLabel}*\nأي آم سبيشل\n\n${head}\n\n${lead}\n\nتصنيف: ${perfLabel}\n\n${url}`;
...
function toggleTimeCalc() {
    ...
    if (isTimeCalcEnabled) {
        ...
        if (label) label.innerHTML = 'قياس الأداء<br><span class="font-medium text-slate-400">يعطيك النظام نظرة سريعة على النتيجة المتوقعة لنهاية الشهر بناءً على أدائك الحالي. مثال: في يوم 28، إذا كان لديك 34 تقييمًا، يتوقع النتيجة 9.7 نقاط — اضغطي الزر للتبديل لوضع كامل الشهر</span>';
    } else {
        if (btn)   { btn.textContent = 'حساب النتيجة'; btn.className = 'w-24 text-[10px] font-bold px-2 py-1 rounded-lg border-2 transition bg-transparent border-blue-500 text-blue-600 flex-shrink-0'; }
...
function setupDateCalculator() {
    const today = new Date(), cur = today.getDate();
...
function calculateTrial() {
    ...
    rb.style.borderColor      = lv.border;
    rb.style.backgroundColor  = `${lv.border}18`;
}
...
function loadAdminData() {
    ...
    const map = {
        admin_bName: d.bName, admin_mName: d.mName, admin_dName: d.dName,
        admin_safety: d.safety, admin_visitors: d.visitors, admin_complaints: d.complaints,
        admin_negative: d.negative, admin_target: d.target,
        admin_startReviews: d.baseReviews, admin_currentReviews: d.baseReviews + d.positive,
...
async function saveAdminData() {
    ...
    const sr  = num('admin_startReviews', branchesData[id].baseReviews);
    const cr  = num('admin_currentReviews', sr + branchesData[id].positive);
    const neg = num('admin_negative');
    ...
    branchesData[id] = {
        ...
        safety: num('admin_safety'), visitors: num('admin_visitors', 1),
        complaints: num('admin_complaints'), positive: pos, negative: neg,
        target: num('admin_target', 50), baseRating: num('admin_baseRating', branchesData[id].baseRating),
        baseReviews: sr, iframeSrc: branchesData[id].iframeSrc
    };
    closeAdmin();
    const dk  = new Date().toISOString().split('T')[0];
    if (!branchHistory[id]) branchHistory[id] = [];
...
function openHistoryModal(branchId) {
    ...
    if (!hist.length) {
        cont.innerHTML = `<p class="text-slate-500 text-center font-bold py-8 bg-white/40 rounded-xl border border-white/50">لا يوجد سجل حتى الآن</p>`;
    } else {
